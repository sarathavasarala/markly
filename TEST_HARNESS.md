...
Daily Brief tracing is opt-in and should remain disabled in automated tests unless a test explicitly mocks the trace sink. The default `BRIEF_TRACE_SINK=noop` path keeps pytest isolated from Langfuse credentials and network calls.
...
### 5. Daily Brief Trace Capture
**Goal**: Daily Brief generation can emit replayable traces without bloating the SQLite database or coupling Signal code to Langfuse.
- **How we handle it**: Signal routes call `services.brief_tracing`, which exposes a Markly-owned tracing interface. The default sink is `noop`; the optional Langfuse sink is selected by environment variables.
- **Verification**: Backend Signal tests run with tracing disabled by default. Trace sink behavior should be tested with mocks when tracing-specific tests are added.

### 6. Local Saved Copy Archiving and Reader View
